...
#endif // IMPORT_CPP_SDK_INTO_IDA
{
        ...
        {
        private:
                template<typename AllocatedArrayElementType>
        ...
        public:
                ...
                inline ArrayElementType* Search(Pred Predicate)
                {
                ...
                template <typename Pred>
                inline const ArrayElementType* Search(Pred Predicate) const
                {
                        for (int32 i = 0; i < NumElements; i++)
                                if (Predicate(Data[i]))
                                        return &Data[i];
                ...
                template <typename Pred>
                inline int32 SearchIndex(Pred Predicate) const
                {
                        for (int32 i = 0; i < NumElements; i++)
                                if (Predicate(Data[i]))
                                        return i;
                ...
                inline void AddGrow(const ArrayElementType& Element)
                {
                        if (GetSlack() <= 0)
                        {
                                int32 NewMax = MaxElements ? MaxElements * 2 : 4;
                                Data = (ArrayElementType*)ContainerRealloc(Data, (int64)NewMax * (int64)ElementSize, (uint32)ElementAlign);
                                MaxElements = NewMax;
                        }
                        Data[NumElements] = Element;
                        NumElements++;
    ...
    public:
                ...
                inline std::string ToString() const
                {
                        if (*this)
                        {
                                if constexpr (sizeof(TCHAR) == 2)
                                        ...
                                        return UtfN::Utf32StringToUtf8String<std::string>(reinterpret_cast<const char32_t*>(Data), NumElements - 1);
                        }
                ...
                inline std::wstring ToWString() const
                {
                        if (*this)
                        {
                                if constexpr (sizeof(TCHAR) == sizeof(wchar_t))
                                        return std::wstring(reinterpret_cast<const wchar_t*>(Data), NumElements - 1);
                                
                                else if constexpr (sizeof(TCHAR) == 2)
                                {
                                        // TCHAR=2, wchar_t=4: convert UTF-16 to UTF-32
                                        const std::u16string u16(reinterpret_cast<const char16_t*>(Data), NumElements - 1);
                                        const std::u32string u32 = UtfN::Utf16StringToUtf32String<std::u32string>(u16);
                                        return std::wstring(reinterpret_cast<const wchar_t*>(u32.data()), u32.size());
                                }
                                else
                                {
                                        // TCHAR=4, wchar_t=2: convert UTF-32 to UTF-16
                                        const std::u32string u32(reinterpret_cast<const char32_t*>(Data), NumElements - 1);
                                        return UtfN::Utf32StringToUtf16String<std::wstring>(u32);
                                }
                        }
        ...
        public:
                ...
                inline       TCHAR* GetData()       { return Data; }
                inline const TCHAR* GetData() const { return Data; }

        public:
                inline bool operator==(const FString& Other) const { return Other ? NumElements == Other.NumElements && std::char_traits<TCHAR>::compare(Data, Other.Data, NumElements - 1) == 0 : false; }
                inline bool operator!=(const FString& Other) const { return Other ? NumElements != Other.NumElements || std::char_traits<TCHAR>::compare(Data, Other.Data, NumElements - 1) != 0 : true; }
        };
        ...
        */
        ...
        public:
                ...
                {
                        this->Data = static_cast<ArrayElementType*>(calloc(1, Size * sizeof(ArrayElementType)));
        ...
        */
        ...
        public:
                ...
                {
                        Data = static_cast<TCHAR*>(calloc(1, Size * sizeof(TCHAR)));
                        NumElements = 0x0;
                        MaxElements = Size;
                }
...
#pragma GCC diagnostic pop
        ...
        public:
                ...
                template <typename Pred>
                TPair<KeyElementType, ValueElementType>* Search(Pred Predicate)
                {
                        for (int32 i = 0; i < Elements.NumAllocated(); i++)
                        {
                                if (!Elements.IsValidIndex(i))
                                        continue;
                                if (Predicate(Elements[i].Key(), Elements[i].Value()))
                                        return &Elements[i];
                        }
                        return nullptr;
                }
                ...
                template <typename Pred>
                TPair<KeyElementType, ValueElementType>* SearchForKey(Pred Predicate)
                {
                        for (int32 i = 0; i < Elements.NumAllocated(); i++)
                        {
                                if (!Elements.IsValidIndex(i))

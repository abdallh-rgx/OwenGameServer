
/*
* SDK generated by MobileDumper-7 & Dumper-7
*
* https://github.com/MJx0/MobileDumper-7
* https://github.com/Encryqed/Dumper-7
*/

// Package: VictoryCrownsUI

#include "Basic.hpp"

#include "VictoryCrownsUI_classes.hpp"
#include "VictoryCrownsUI_parameters.hpp"


SDK_NAMESPACE_START

// Function VictoryCrownsUI.VictoryCrownsPreGameWidget.FadeOutTimerFinished
// (Final, Native, Private)

void UVictoryCrownsPreGameWidget::FadeOutTimerFinished()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("VictoryCrownsPreGameWidget", "FadeOutTimerFinished");

	auto Flgs = Func->FunctionFlags;
	Func->FunctionFlags |= 0x400;

	UObject::ProcessEvent(Func, nullptr);

	Func->FunctionFlags = Flgs;
}


// Function VictoryCrownsUI.VictoryCrownsPreGameWidget.HandleGamePhaseStepChanged
// (Final, Native, Private, HasOutParams)
// Parameters:
// const TScriptInterface<class IFortSafeZoneInterface>& SafeZoneInterface (ConstParm, Parm, OutParm, ZeroConstructor, ReferenceParm, IsPlainOldData, NoDestructor, UObjectWrapper, HasGetValueTypeHash, NativeAccessSpecifierPublic)
// const EEAthenaGamePhaseStep GamePhaseStep (ConstParm, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)

void UVictoryCrownsPreGameWidget::HandleGamePhaseStepChanged(const TScriptInterface<class IFortSafeZoneInterface>& SafeZoneInterface, const EEAthenaGamePhaseStep GamePhaseStep)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("VictoryCrownsPreGameWidget", "HandleGamePhaseStepChanged");

	Params::UVictoryCrownsPreGameWidget_HandleGamePhaseStepChanged Parms{};

	Parms.SafeZoneInterface = SafeZoneInterface;
	Parms.GamePhaseStep = GamePhaseStep;

	auto Flgs = Func->FunctionFlags;
	Func->FunctionFlags |= 0x400;

	UObject::ProcessEvent(Func, &Parms);

	Func->FunctionFlags = Flgs;
}


// Function VictoryCrownsUI.VictoryCrownsPreGameWidget.OnButtonClose
// (Event, Protected, BlueprintCallable, BlueprintEvent)

void UVictoryCrownsPreGameWidget::OnButtonClose()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("VictoryCrownsPreGameWidget", "OnButtonClose");

	UObject::ProcessEvent(Func, nullptr);
}


// Function VictoryCrownsUI.VictoryCrownsPreGameWidget.OnButtonCloseProgress
// (Event, Protected, BlueprintEvent)
// Parameters:
// float Progress (Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)

void UVictoryCrownsPreGameWidget::OnButtonCloseProgress(float Progress)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("VictoryCrownsPreGameWidget", "OnButtonCloseProgress");

	Params::UVictoryCrownsPreGameWidget_OnButtonCloseProgress Parms{};

	Parms.Progress = Progress;

	UObject::ProcessEvent(Func, &Parms);
}


// Function VictoryCrownsUI.VictoryCrownsPreGameWidget.OnCloseActionsComplete
// (Final, Native, Protected, BlueprintCallable)

void UVictoryCrownsPreGameWidget::OnCloseActionsComplete()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("VictoryCrownsPreGameWidget", "OnCloseActionsComplete");

	auto Flgs = Func->FunctionFlags;
	Func->FunctionFlags |= 0x400;

	UObject::ProcessEvent(Func, nullptr);

	Func->FunctionFlags = Flgs;
}


// Function VictoryCrownsUI.VictoryCrownsPreGameWidget.OnInputMethodChanged
// (Event, Protected, BlueprintEvent)
// Parameters:
// EECommonInputType CurrentInput (Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)

void UVictoryCrownsPreGameWidget::OnInputMethodChanged(EECommonInputType CurrentInput)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("VictoryCrownsPreGameWidget", "OnInputMethodChanged");

	Params::UVictoryCrownsPreGameWidget_OnInputMethodChanged Parms{};

	Parms.CurrentInput = CurrentInput;

	UObject::ProcessEvent(Func, &Parms);
}


// Function VictoryCrownsUI.VictoryCrownsPreGameWidget.RebuildBearerList
// (Final, Native, Private, HasOutParams)
// Parameters:
// const TArray<class AFortPlayerStateAthena*>& CrownBearerList (ConstParm, Parm, OutParm, ZeroConstructor, ReferenceParm, NativeAccessSpecifierPublic)

void UVictoryCrownsPreGameWidget::RebuildBearerList(const TArray<class AFortPlayerStateAthena*>& CrownBearerList)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("VictoryCrownsPreGameWidget", "RebuildBearerList");

	Params::UVictoryCrownsPreGameWidget_RebuildBearerList Parms{};

	Parms.CrownBearerList = std::move(CrownBearerList);

	auto Flgs = Func->FunctionFlags;
	Func->FunctionFlags |= 0x400;

	UObject::ProcessEvent(Func, &Parms);

	Func->FunctionFlags = Flgs;
}


// Function VictoryCrownsUI.VictoryCrownsPreGameWidget.RegisterCloseButtonInput
// (Final, Native, Private)

void UVictoryCrownsPreGameWidget::RegisterCloseButtonInput()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("VictoryCrownsPreGameWidget", "RegisterCloseButtonInput");

	auto Flgs = Func->FunctionFlags;
	Func->FunctionFlags |= 0x400;

	UObject::ProcessEvent(Func, nullptr);

	Func->FunctionFlags = Flgs;
}


// Function VictoryCrownsUI.VictoryCrownsPreGameWidget.StartFadeOut
// (Event, Protected, BlueprintEvent)

void UVictoryCrownsPreGameWidget::StartFadeOut()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("VictoryCrownsPreGameWidget", "StartFadeOut");

	UObject::ProcessEvent(Func, nullptr);
}


// Function VictoryCrownsUI.VictoryCrownsPreGameWidget.UnregisterCloseButtonInput
// (Final, Native, Private)

void UVictoryCrownsPreGameWidget::UnregisterCloseButtonInput()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("VictoryCrownsPreGameWidget", "UnregisterCloseButtonInput");

	auto Flgs = Func->FunctionFlags;
	Func->FunctionFlags |= 0x400;

	UObject::ProcessEvent(Func, nullptr);

	Func->FunctionFlags = Flgs;
}


// Function VictoryCrownsUI.VictoryCrownsIndicator.OnDistanceChanged
// (Event, Protected, BlueprintEvent)
// Parameters:
// int32 Distance (Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)

void UVictoryCrownsIndicator::OnDistanceChanged(int32 Distance)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("VictoryCrownsIndicator", "OnDistanceChanged");

	Params::UVictoryCrownsIndicator_OnDistanceChanged Parms{};

	Parms.Distance = Distance;

	UObject::ProcessEvent(Func, &Parms);
}


// Function VictoryCrownsUI.VictoryCrownsIndicator.OnScreenClampChanged
// (Event, Protected, BlueprintEvent)
// Parameters:
// bool bIsClamped (Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)

void UVictoryCrownsIndicator::OnScreenClampChanged(bool bIsClamped)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("VictoryCrownsIndicator", "OnScreenClampChanged");

	Params::UVictoryCrownsIndicator_OnScreenClampChanged Parms{};

	Parms.bIsClamped = bIsClamped;

	UObject::ProcessEvent(Func, &Parms);
}


// Function VictoryCrownsUI.VictoryCrownsIndicator.GetIndicatedPlayerStateAthena
// (Final, Native, Public, BlueprintCallable, BlueprintPure, Const)
// Parameters:
// const class AFortPlayerStateAthena* ReturnValue (ConstParm, Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)

const class AFortPlayerStateAthena* UVictoryCrownsIndicator::GetIndicatedPlayerStateAthena() const
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("VictoryCrownsIndicator", "GetIndicatedPlayerStateAthena");

	Params::UVictoryCrownsIndicator_GetIndicatedPlayerStateAthena Parms{};

	auto Flgs = Func->FunctionFlags;
	Func->FunctionFlags |= 0x400;

	UObject::ProcessEvent(Func, &Parms);

	Func->FunctionFlags = Flgs;

	return Parms.ReturnValue;
}


// Function VictoryCrownsUI.VictoryCrownsBearerInfoBase.OnSetPlayerState
// (Event, Protected, BlueprintEvent)
// Parameters:
// class AFortPlayerStateAthena* PSA (Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)

void UVictoryCrownsBearerInfoBase::OnSetPlayerState(class AFortPlayerStateAthena* PSA)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("VictoryCrownsBearerInfoBase", "OnSetPlayerState");

	Params::UVictoryCrownsBearerInfoBase_OnSetPlayerState Parms{};

	*reinterpret_cast<void**>(&Parms.PSA) = reinterpret_cast<void*>(PSA);

	UObject::ProcessEvent(Func, &Parms);
}


// Function VictoryCrownsUI.VictoryCrownsBearerInfoBase.SetPlayerState
// (Final, Native, Public, BlueprintCallable)
// Parameters:
// class AFortPlayerStateAthena* InPlayerState (Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)

void UVictoryCrownsBearerInfoBase::SetPlayerState(class AFortPlayerStateAthena* InPlayerState)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("VictoryCrownsBearerInfoBase", "SetPlayerState");

	Params::UVictoryCrownsBearerInfoBase_SetPlayerState Parms{};

	*reinterpret_cast<void**>(&Parms.InPlayerState) = reinterpret_cast<void*>(InPlayerState);

	auto Flgs = Func->FunctionFlags;
	Func->FunctionFlags |= 0x400;

	UObject::ProcessEvent(Func, &Parms);

	Func->FunctionFlags = Flgs;
}


// Function VictoryCrownsUI.VictoryCrownsPlayerInfoWidget.HandleCrownBearerListChanged
// (Final, Native, Protected, HasOutParams)
// Parameters:
// const TArray<class AFortPlayerStateAthena*>& CrownBearerList (ConstParm, Parm, OutParm, ZeroConstructor, ReferenceParm, NativeAccessSpecifierPublic)

void UVictoryCrownsPlayerInfoWidget::HandleCrownBearerListChanged(const TArray<class AFortPlayerStateAthena*>& CrownBearerList)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("VictoryCrownsPlayerInfoWidget", "HandleCrownBearerListChanged");

	Params::UVictoryCrownsPlayerInfoWidget_HandleCrownBearerListChanged Parms{};

	Parms.CrownBearerList = std::move(CrownBearerList);

	auto Flgs = Func->FunctionFlags;
	Func->FunctionFlags |= 0x400;

	UObject::ProcessEvent(Func, &Parms);

	Func->FunctionFlags = Flgs;
}


// Function VictoryCrownsUI.VictoryCrownsPlayerInfoWidget.OnPlayerVictoryCrownStatusChanged
// (Event, Protected, BlueprintEvent)
// Parameters:
// const bool bHasCrown (ConstParm, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)

void UVictoryCrownsPlayerInfoWidget::OnPlayerVictoryCrownStatusChanged(const bool bHasCrown)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("VictoryCrownsPlayerInfoWidget", "OnPlayerVictoryCrownStatusChanged");

	Params::UVictoryCrownsPlayerInfoWidget_OnPlayerVictoryCrownStatusChanged Parms{};

	Parms.bHasCrown = bHasCrown;

	UObject::ProcessEvent(Func, &Parms);
}


// Function VictoryCrownsUI.FortUIManagerComponent_VictoryCrowns.HandleGamePhaseStepChanged
// (Final, Native, Protected, HasOutParams)
// Parameters:
// const TScriptInterface<class IFortSafeZoneInterface>& SafeZoneInterface (ConstParm, Parm, OutParm, ZeroConstructor, ReferenceParm, IsPlainOldData, NoDestructor, UObjectWrapper, HasGetValueTypeHash, NativeAccessSpecifierPublic)
// const EEAthenaGamePhaseStep GamePhaseStep (ConstParm, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)

void UFortUIManagerComponent_VictoryCrowns::HandleGamePhaseStepChanged(const TScriptInterface<class IFortSafeZoneInterface>& SafeZoneInterface, const EEAthenaGamePhaseStep GamePhaseStep)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FortUIManagerComponent_VictoryCrowns", "HandleGamePhaseStepChanged");

	Params::UFortUIManagerComponent_VictoryCrowns_HandleGamePhaseStepChanged Parms{};

	Parms.SafeZoneInterface = SafeZoneInterface;
	Parms.GamePhaseStep = GamePhaseStep;

	auto Flgs = Func->FunctionFlags;
	Func->FunctionFlags |= 0x400;

	UObject::ProcessEvent(Func, &Parms);

	Func->FunctionFlags = Flgs;
}


// Function VictoryCrownsUI.FortUIManagerComponent_VictoryCrowns.RebuildBearerIndicators
// (Final, Native, Protected, HasOutParams)
// Parameters:
// const TArray<class AFortPlayerStateAthena*>& CrownBearerList (ConstParm, Parm, OutParm, ZeroConstructor, ReferenceParm, NativeAccessSpecifierPublic)

void UFortUIManagerComponent_VictoryCrowns::RebuildBearerIndicators(const TArray<class AFortPlayerStateAthena*>& CrownBearerList)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FortUIManagerComponent_VictoryCrowns", "RebuildBearerIndicators");

	Params::UFortUIManagerComponent_VictoryCrowns_RebuildBearerIndicators Parms{};

	Parms.CrownBearerList = std::move(CrownBearerList);

	auto Flgs = Func->FunctionFlags;
	Func->FunctionFlags |= 0x400;

	UObject::ProcessEvent(Func, &Parms);

	Func->FunctionFlags = Flgs;
}


SDK_NAMESPACE_END

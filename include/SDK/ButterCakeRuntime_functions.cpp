
/*
* SDK generated by MobileDumper-7 & Dumper-7
*
* https://github.com/MJx0/MobileDumper-7
* https://github.com/Encryqed/Dumper-7
*/

// Package: ButterCakeRuntime

#include "Basic.hpp"

#include "ButterCakeRuntime_classes.hpp"
#include "ButterCakeRuntime_parameters.hpp"


SDK_NAMESPACE_START

// Function ButterCakeRuntime.FortButterCakeControlRig.GetGroundHitLocationAt
// (Final, Native, Public, HasDefaults, BlueprintCallable)
// Parameters:
// const int32 Index_0 (ConstParm, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)
// struct FVector ReturnValue (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)

struct FVector UFortButterCakeControlRig::GetGroundHitLocationAt(const int32 Index_0)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FortButterCakeControlRig", "GetGroundHitLocationAt");

	Params::UFortButterCakeControlRig_GetGroundHitLocationAt Parms{};

	Parms.Index_0 = Index_0;

	auto Flgs = Func->FunctionFlags;
	Func->FunctionFlags |= 0x400;

	UObject::ProcessEvent(Func, &Parms);

	Func->FunctionFlags = Flgs;

	return Parms.ReturnValue;
}


// Function ButterCakeRuntime.FortButterCakeControlRig.GetGroundHitNormalAt
// (Final, Native, Public, HasDefaults, BlueprintCallable)
// Parameters:
// const int32 Index_0 (ConstParm, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)
// struct FVector ReturnValue (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)

struct FVector UFortButterCakeControlRig::GetGroundHitNormalAt(const int32 Index_0)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FortButterCakeControlRig", "GetGroundHitNormalAt");

	Params::UFortButterCakeControlRig_GetGroundHitNormalAt Parms{};

	Parms.Index_0 = Index_0;

	auto Flgs = Func->FunctionFlags;
	Func->FunctionFlags |= 0x400;

	UObject::ProcessEvent(Func, &Parms);

	Func->FunctionFlags = Flgs;

	return Parms.ReturnValue;
}


// Function ButterCakeRuntime.FortAIAnimInstance_ButterCake.ComputeFootPhase
// (Final, Native, Public, BlueprintCallable, BlueprintPure)
// Parameters:
// EEFortButterCakeFootPhase ReturnValue (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)

EEFortButterCakeFootPhase UFortAIAnimInstance_ButterCake::ComputeFootPhase()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FortAIAnimInstance_ButterCake", "ComputeFootPhase");

	Params::UFortAIAnimInstance_ButterCake_ComputeFootPhase Parms{};

	auto Flgs = Func->FunctionFlags;
	Func->FunctionFlags |= 0x400;

	UObject::ProcessEvent(Func, &Parms);

	Func->FunctionFlags = Flgs;

	return Parms.ReturnValue;
}


// Function ButterCakeRuntime.FortAIAnimInstance_ButterCake.ComputeLeanAngleByVelocity
// (Final, Native, Public, BlueprintCallable, BlueprintPure)
// Parameters:
// float ReturnValue (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)

float UFortAIAnimInstance_ButterCake::ComputeLeanAngleByVelocity()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FortAIAnimInstance_ButterCake", "ComputeLeanAngleByVelocity");

	Params::UFortAIAnimInstance_ButterCake_ComputeLeanAngleByVelocity Parms{};

	auto Flgs = Func->FunctionFlags;
	Func->FunctionFlags |= 0x400;

	UObject::ProcessEvent(Func, &Parms);

	Func->FunctionFlags = Flgs;

	return Parms.ReturnValue;
}


// Function ButterCakeRuntime.FortAIAnimInstance_ButterCake.SetFootPhaseMembers
// (Final, Native, Public, BlueprintCallable)

void UFortAIAnimInstance_ButterCake::SetFootPhaseMembers()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FortAIAnimInstance_ButterCake", "SetFootPhaseMembers");

	auto Flgs = Func->FunctionFlags;
	Func->FunctionFlags |= 0x400;

	UObject::ProcessEvent(Func, nullptr);

	Func->FunctionFlags = Flgs;
}


// Function ButterCakeRuntime.ButterCakeUnstuckComponent.HandleAthenaGamePhaseChanged
// (Final, Native, Private)
// Parameters:
// EEAthenaGamePhase GamePhase (Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)

void UButterCakeUnstuckComponent::HandleAthenaGamePhaseChanged(EEAthenaGamePhase GamePhase)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("ButterCakeUnstuckComponent", "HandleAthenaGamePhaseChanged");

	Params::UButterCakeUnstuckComponent_HandleAthenaGamePhaseChanged Parms{};

	Parms.GamePhase = GamePhase;

	auto Flgs = Func->FunctionFlags;
	Func->FunctionFlags |= 0x400;

	UObject::ProcessEvent(Func, &Parms);

	Func->FunctionFlags = Flgs;
}


// Function ButterCakeRuntime.ButterCakeUnstuckComponent.ResetUnstuckLocationSamples
// (Final, BlueprintAuthorityOnly, Native, Public, BlueprintCallable)

void UButterCakeUnstuckComponent::ResetUnstuckLocationSamples()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("ButterCakeUnstuckComponent", "ResetUnstuckLocationSamples");

	auto Flgs = Func->FunctionFlags;
	Func->FunctionFlags |= 0x400;

	UObject::ProcessEvent(Func, nullptr);

	Func->FunctionFlags = Flgs;
}


// Function ButterCakeRuntime.FortButterCakeComponent_Telemetry.OnBlowHoleUsed
// (Final, Native, Public, BlueprintCallable)

void UFortButterCakeComponent_Telemetry::OnBlowHoleUsed()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FortButterCakeComponent_Telemetry", "OnBlowHoleUsed");

	auto Flgs = Func->FunctionFlags;
	Func->FunctionFlags |= 0x400;

	UObject::ProcessEvent(Func, nullptr);

	Func->FunctionFlags = Flgs;
}


// Function ButterCakeRuntime.FortButterCakeComponent_Telemetry.OnEnterBerserk
// (Final, Native, Public, BlueprintCallable)
// Parameters:
// const class AController* Instigator (ConstParm, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)

void UFortButterCakeComponent_Telemetry::OnEnterBerserk(const class AController* Instigator)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FortButterCakeComponent_Telemetry", "OnEnterBerserk");

	Params::UFortButterCakeComponent_Telemetry_OnEnterBerserk Parms{};

	*reinterpret_cast<const void**>(&Parms.Instigator) = reinterpret_cast<const void*>(Instigator);

	auto Flgs = Func->FunctionFlags;
	Func->FunctionFlags |= 0x400;

	UObject::ProcessEvent(Func, &Parms);

	Func->FunctionFlags = Flgs;
}


// Function ButterCakeRuntime.FortButterCakeComponent_Telemetry.OnItemsEaten
// (Final, Native, Public, BlueprintCallable)
// Parameters:
// const int32 ItemsCount (ConstParm, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)

void UFortButterCakeComponent_Telemetry::OnItemsEaten(const int32 ItemsCount)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FortButterCakeComponent_Telemetry", "OnItemsEaten");

	Params::UFortButterCakeComponent_Telemetry_OnItemsEaten Parms{};

	Parms.ItemsCount = ItemsCount;

	auto Flgs = Func->FunctionFlags;
	Func->FunctionFlags |= 0x400;

	UObject::ProcessEvent(Func, &Parms);

	Func->FunctionFlags = Flgs;
}


// Function ButterCakeRuntime.FortButterCakeComponent_Telemetry.OnItemsSneezed
// (Final, Native, Public, BlueprintCallable)
// Parameters:
// const int32 ItemsCount (ConstParm, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)

void UFortButterCakeComponent_Telemetry::OnItemsSneezed(const int32 ItemsCount)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FortButterCakeComponent_Telemetry", "OnItemsSneezed");

	Params::UFortButterCakeComponent_Telemetry_OnItemsSneezed Parms{};

	Parms.ItemsCount = ItemsCount;

	auto Flgs = Func->FunctionFlags;
	Func->FunctionFlags |= 0x400;

	UObject::ProcessEvent(Func, &Parms);

	Func->FunctionFlags = Flgs;
}


// Function ButterCakeRuntime.FortButterCakeComponent_Telemetry.OnLured
// (Final, Native, Public, BlueprintCallable)

void UFortButterCakeComponent_Telemetry::OnLured()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("FortButterCakeComponent_Telemetry", "OnLured");

	auto Flgs = Func->FunctionFlags;
	Func->FunctionFlags |= 0x400;

	UObject::ProcessEvent(Func, nullptr);

	Func->FunctionFlags = Flgs;
}


SDK_NAMESPACE_END

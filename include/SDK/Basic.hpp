...
#include <type_traits>
#include <cmath>
// <format> removed: it was included but never used, and the libc++ shipped with
// NDK r25 (clang 14) does not provide <format>. Nothing in the SDK uses std::format.
// <cmath>: std::sqrt / std::fmod are used by the generated *_structs.hpp files;
// newer libc++ (NDK r26+) pulls <cmath> in transitively, r25c's does not.
// <concepts>/<compare>: required for std::invocable, std::equality_comparable and
// std::strong_ordering below. Newer libc++ (NDK r26+) pulls them in transitively;
// r25c's libc++ 14 does not, so they are included explicitly.
#include <concepts>
#include <compare>
...
namespace InSDKUtils
{
        ...
        // std::invocable requires <concepts> support that NDK r25c (libc++ 14) does not
        // ship; std::is_invocable_v (C++17 <type_traits>) is the portable equivalent.
#if defined(__cpp_lib_concepts)
        template<typename FuncType, typename... ParamTypes>
        requires std::invocable<FuncType, ParamTypes...>
#else
        template<typename FuncType, typename... ParamTypes>
        requires std::is_invocable_v<FuncType, ParamTypes...>
#endif
